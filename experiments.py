command =  "/home/aifs2/dwr29/search/tiles/15md_solver {algorithm} -aspect {aspect} -cost {cost} -wt {subopt} -width {width} < /home/aifs2/group/data/tiles_instances/korf/4/4/{korfnum} > /home/aifs2/dwr29/search/results/{outfile}" 
widths = [4**i for i in range(2, 6)]  # [16, 64, 256, 1024]
weights = [1.03, 1.05, 1.07, 1.1, 1.2, 1.3, 1.4, 1.5, 1.7,  2, 3, 5, 10]
            ...
            for weight in alg_weights:
                for aspect in alg_aspects:
                    for inst in korf_nums:
                        ofname = "_".join([algorithm, cost, str(weight), str(width), str(aspect), str(inst)])  
                        print(command.format(algorithm = algorithm, cost = cost, subopt = weight, width = width, aspect = aspect, korfnum = inst, outfile =ofname))
