...
template <class D> struct BSBSFILL : public SearchAlgorithm<D> {
	...
	BSBSFILL(int argc, const char *argv[]) :
	...
	Node *select_node(bool pickf = false) {
	  	Node *bestDHat = NULL;
		if(!focal.empty()) {
	...
	void search(D &d, typename D::State &s0) {
		...
		while (!cleanup.empty() && !SearchAlgorithm<D>::limit()) {
			...
			if (c == 0){
				...
				while (!cleanup.empty() && !SearchAlgorithm<D>::limit() && focal.size() < width) {
					n_astar_expansions++;
					astar_exps++;
